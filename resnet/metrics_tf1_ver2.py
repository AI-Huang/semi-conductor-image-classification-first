...
class AUC0(Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        Returns:
          Update op.
        """
        # y_true = K.cast(y_true, self.dtype)  # K.cast
        # y_pred = K.cast(y_pred, self.dtype)
        # y_true = K.map_fn(lambda v: 1 - v, y_true)  # tf.map_fn
        # y_pred = K.map_fn(lambda v: 1 - v, y_pred)
        y_true = math_ops.subtract(1.0, y_true)
        y_pred = math_ops.subtract(1.0, y_pred)
