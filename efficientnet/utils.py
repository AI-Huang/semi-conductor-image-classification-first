...
def drop_connect(inputs, is_training, survival_prob):
    ...
    random_tensor += tf.random_uniform([batch_size,
...
def archive_ckpt(ckpt_eval, ckpt_objective, ckpt_path):
    ...
    if saved_objective > ckpt_objective:
        logging.info('Ckpt %s is worse than %s',
    ...
    dst_dir = os.path.join(".", 'archive')  # ckpt_dir
...
class EvalCkptDriver(object):
    ...
    def eval_imagenet(self, ckpt_dir, imagenet_eval_glob,
        ...
        """
        imagenet_val_labels = [int(i)
                               for i in tf.gfile.GFile(imagenet_eval_label)]
        ...
        print(
            'Final: top1_acc = {:4.2f}%  top5_acc = {:4.2f}%'.format(top1, top5))
        return top1, top5
