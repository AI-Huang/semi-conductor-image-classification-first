...
flags.DEFINE_bool(
    'use_tpu', default=False,  # True to use TPU
...
flags.DEFINE_string(
    'mode', default='train_and_eval',  # train_and_eval
    help='One of {"train_and_eval", "train", "eval"}.')

flags.DEFINE_string(
    'augment_name', default="autoaugment",
...
flags.DEFINE_integer(
    'eval_batch_size', default=1000, help='Batch size for evaluation.')  # not 1024

flags.DEFINE_integer(
    'num_train_images', default=24000, help='Size of training data set.')

flags.DEFINE_integer(
    'num_eval_images', default=6000, help='Size of evaluation data set.')  # half the size of validation set 6000

flags.DEFINE_integer(
    'steps_per_eval', default=750,  # half of 1500 steps for a whole epoch
...
flags.DEFINE_integer(
    'num_label_classes', default=2, help='Number of classes, at least 2')  # 1000 for ImageNet
...
flags.DEFINE_float(
    'base_learning_rate',
    default=0.256,
    help=('Base learning rate when train batch size is 256.'))  # base learning rate is 0.016 for batch size 256, now 16
    ...
    if FLAGS.mode == 'eval':
        ...
        for ckpt in tf.train.checkpoints_iterator(
            ...
            try:
                ...
                utils.archive_ckpt(
                    eval_results, eval_results['auc'], ckpt)
        ...
        if FLAGS.mode == 'train':
            ...
            while current_step < FLAGS.train_steps:
                ...
                # archive_ckpt = "./archive"
                utils.archive_ckpt(
                    eval_results, eval_results['auc'], ckpt)  # archive_ckpt 的第二个参数是 ckpt_objective
